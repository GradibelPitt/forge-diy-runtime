...
启动器会检查 `main` 的最新提交；首次安装时下载完整运行包，之后按已安装的提交 SHA 比较差异，只下载并覆盖变更文件（包括删除、重命名），不因修改一张卡或文档重新下载整包。无更新时复用已有安装；普通卡牌更新不重新扫描大 JAR，仅在 JAR 或其声明哈希变化时进行 JAR 校验。首次安装、本地运行包损坏、历史重写或 GitHub 差异列表达到 300 个文件上限时，使用完整运行包。启动器自动获取本机架构的 Java 17，同步 DIY 卡牌、图片、音乐后启动桌面版 Forge；检查更新失败且本地运行包完整时会提示并使用本地版本。游戏运行期间请保留终端窗口。

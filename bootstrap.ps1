...
function Resolve-ForgeStorageMigrationHelper {
    param([string]$HelperPath = (Join-Path $PSScriptRoot 'tools\storage_migration.ps1'))

    # SHA-256 of UTF-8 source with LF line endings and no BOM. Update this pin
    # together with tools/storage_migration.ps1; Git's CRLF checkout is harmless.
    $expectedHash = '6975d0549d7b1d59286c85b807574c2c32a7f13592529acca21bbc97097f1691'
    $temporaryDirectory = $null
    try {
        if (-not (Test-Path -LiteralPath $HelperPath -PathType Leaf)) {
            # The .cmd/.bat entry points download bootstrap.ps1 to TEMP, so a
            # sibling tools directory is not always available before migration.
            $temporaryDirectory = Join-Path ([IO.Path]::GetTempPath()) ('forge-diy-storage-' + [Guid]::NewGuid().ToString('N'))
            New-Item -ItemType Directory -Path $temporaryDirectory | Out-Null
            $HelperPath = Join-Path $temporaryDirectory 'storage_migration.ps1'
            $previousProtocol = [Net.ServicePointManager]::SecurityProtocol
            try {
                [Net.ServicePointManager]::SecurityProtocol = $previousProtocol -bor [Net.SecurityProtocolType]::Tls12
                Invoke-WebRequest -UseBasicParsing -Uri 'https://raw.githubusercontent.com/GradibelPitt/forge-diy-runtime/main/tools/storage_migration.ps1' -OutFile $HelperPath
            } finally {
        ...
        try {
            $actualHash = [BitConverter]::ToString($sha256.ComputeHash([Text.Encoding]::UTF8.GetBytes($normalizedSource))).Replace('-', '').ToLowerInvariant()
        } finally { $sha256.Dispose() }
        if ($actualHash -ne $expectedHash) {
            throw '存储迁移脚本版本不匹配或下载不完整。请重新获取最新 starter 启动文件后重试。'
        }
        return [pscustomobject]@{ Path = $HelperPath; TemporaryDirectory = $temporaryDirectory }
    } catch {
...
function Initialize-ForgeStorage {
    ...
    try {
        ...
        if ($migrationModule) { Remove-Module $migrationModule -Force -ErrorAction SilentlyContinue }
